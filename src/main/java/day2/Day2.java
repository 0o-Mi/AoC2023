...
public class Day2 {
    ...
    // turns the roundsString into a SummarizedGame - maximum color conversions
    ...
    private static Integer getMaxSummarizedColorValue(List<String[]> rounds, String color) {
        return rounds.stream().map(array -> Arrays.stream(array)
                .filter(colorValue -> colorValue.contains(color)) // filters conversions in round to one color
